            ...
            <div key={ei} className={`rounded-2xl border overflow-hidden transition-all ${exDone ? "border-emerald-800/50 bg-emerald-950/20" : "border-zinc-800 bg-zinc-900"}`}>
              ...
              {!isCollapsed && (
                <div className="px-4 pb-4 space-y-2">
                  {/* Column headers */}
                  <div className="grid grid-cols-[28px_1fr_1fr_32px] gap-2 text-xs text-zinc-600 px-1 mb-1">
                    <span>Set</span>
                    <span>Weight (kg)</span>
                    <span>Reps</span>
                    <span />
                  ...
                  {ex.sets.map((set, si) => (
                    <div key={si} className={`grid grid-cols-[28px_1fr_1fr_32px] gap-2 items-center transition-opacity ${set.done ? "opacity-60" : ""}`}>
                      <span className="text-xs text-zinc-500 text-center">{set.setNumber}</span>
                      <input
                        type="number"
                        inputMode="decimal"
                        placeholder="0"
