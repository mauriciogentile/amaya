...
export default function WorkoutSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-950 items-center justify-center px-4 pb-24">
      <div className="max-w-lg w-full space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-zinc-500 text-sm truncate">{workout.name}</p>
        </div>
        ...
        {workout.exercises?.length > 0 && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 space-y-3">
            <p className="text-xs text-zinc-500 uppercase tracking-widest font-semibold">Exercises</p>
            {workout.exercises.map((ex: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-white font-medium">{ex.exerciseName}</p>
                    <p className="text-xs text-zinc-500">{doneSets.length} sets</p>
                  </div>
